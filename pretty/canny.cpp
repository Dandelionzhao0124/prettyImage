...
//*****************************
void gaussianBlur(unsigned char* grayData, unsigned char* gaussblur, int width, int height) {
	...
	memcpy(gaussblur, grayData, width*height*sizeof(unsigned char));
...
//****************************
void sobel(unsigned char* grayData, int width, int height) {
	...
	for (int j = 0; j < height; j++) {
		for (int i = 0; i < width; i++) {
			...
			grayData[j*width+i] =  edge_magnitude[j * width + i];
		}
	}
}
...
//*****************************
void noMax(unsigned char* grayData, int width, int height) {
...
void hysteresis(unsigned char* grayData, int width, int height, int lowThresh, int highThresh) {
...
unsigned char* canny(unsigned char* grayData, int width, int height) {
	...
	unsigned char* gaussblur = new unsigned char[height*width];
	gaussianBlur(grayData, gaussblur, width, height);
	sobel(gaussblur, width, height);
	noMax(gaussblur, width, height);
	hysteresis(gaussblur, width, height, lowThresh, highThresh);
